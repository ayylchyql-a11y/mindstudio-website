...
rail.addEventListener('click', (e) => { const a = e.target.closest('a'); if (!a) return; e.preventDefault(); try { history.replaceState(null, '', '#' + a.dataset.id); } catch (_) {} /* sandboxed iframe: opaque origin */ if (a.dataset.id === cur) return openPanel(); go(a.dataset.id); });

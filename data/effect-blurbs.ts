...
const blurbs: Record<string, string> = {
  ...
  "dashboard-keyboard":
    "A back office built for people who never touch the mouse: a neutral, dense layout of tables and 1px lines where a highlight bar walks the rows from the keyboard, a side panel opens on Enter, orders advance with a single key and a command palette runs everything else. Riders are assigned from a pill of faces, the kitchen queue is reordered by dragging, and the closing checklist collapses when the last box is ticked. The tone of Linear and other operator tools, on live restaurant data.",
  "dashboard-terminal":
    "The whole restaurant in a green-on-black terminal. Charts are drawn with block characters from the real numbers, a log streams at the bottom like a server tail, and the command line is not decoration: you can list orders, confirm them, mark a dish sold out or pause the shop by typing, with tab completion and history. A style for developer tools, status pages and anything that wants to feel like infrastructure rather than an app.",
  "dashboard-brutal":
    "Thick black borders, hard offset shadows, flat candy colours and stickers set slightly off square: a back office that looks like a poster and feels like a machine. Big numbers you can read from across the kitchen, a gauge you spin with a finger, order cards whose actions fan out from a single button, a channel filter that physically shoves its neighbours aside, and a slide-to-confirm to close the till. The neo-brutalist look, applied to a working admin panel.",
  "dashboard-mobile-glass":
    "The owner’s phone: the same back office reduced to a pocket app with a translucent glass tab bar. Pull the revenue card to refresh and watch the droplets fuse, slide a tray of services out from under the orders card, open the channels like an accordion, drag a summary strip into a full panel and tick orders to fill the shift card. Every gesture is the real one, at phone scale, on the restaurant’s live menu and figures.",
  "dashboard-dark-rail":
